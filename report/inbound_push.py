...
# วัดซ้ำหลังแก้เป็น 60 วิ + ลบ index: เหลือ ~5,000/ชม. = ~72k/วัน ซึ่งยัง "พอดีเกินไป" —
# รวมกับ sightings/tracks/heartbeat แล้ว ~82k จาก 100k เหลือที่ว่าง 18% ไม่พอรองรับวันจราจรหนา
# หรือ backlog รอบใหม่ตอนเน็ตหลุด (ครั้งนี้ backlog 19k แถวกินโควตาไป ~39k ทีเดียว)
# 120 วิ = ~36k/วัน รวมทุกตาราง ~46k เหลือที่ว่างครึ่งหนึ่งให้เหตุการณ์ไม่คาดฝัน
# ETA คิดเป็นนาทีอยู่แล้ว ความสดระดับ 2 นาทีจึงไม่เปลี่ยนสิ่งที่ผู้ใช้เห็น ตั้ง env ทับได้ถ้าอยากถี่กว่า
PUSH_INTERVAL_S = int(os.environ.get("PUSH_INTERVAL_S", "120"))
