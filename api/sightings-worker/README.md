...
รูปแบบ `tz` ที่รับ: `+07:00` · `+0700` · `7` · `-05:30` · `Z` ทุก response สะท้อนกลับเป็น `tz`
รูปมาตรฐาน + `tz_offset_min` (ตัวเลขล้วน) ซึ่งสร้างจาก offset ที่ตีความแล้ว **ไม่ใช่**สตริงดิบ —
เพราะ `?tz=+07:00` ที่ไม่ได้ encode จะโดนถอด `+` เป็นช่องว่างตามมาตรฐาน URL (ค่ายังคำนวณถูก
แต่ถ้าสะท้อนดิบกลับไปจะกลายเป็น `07:00`). ส่ง `%2B07:00` หรือ `7` ก็ได้ผลเหมือนกัน
